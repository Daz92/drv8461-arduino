...
/// \file DRV8461s.h
///
/// This is the main header file for the DRV8461s library,
/// a library for controlling the DRV8461s stepper motor driver.
///
/// For more information about this library, see:
///
///   https://github.com/pololu/DRV8461s-arduino
///
...
enum class DRV8461sRegAddr: uint8_t
{
  FAULT = 0x00,
...
/// interface of a DRV8461s stepper motor controller IC.
///
...
public: 
  ...
  ///
  /// You must use a chip select pin; the DRV8461s requires it.
  ...
  /// Reads the register at the given address and returns its raw value.
  uint16_t readReg(DRV8461sRegAddr address)
  {
  ...
  uint8_t writeReg(uint8_t address, uint8_t value)
  {
    ...
    /// Writes the specified value to a register.
  void writeReg(DRV8461sRegAddr address, uint8_t value)
  {
...
public: 
  ...
  /// register with DRV8461s::readFault(), except the upper two bits are always
  /// 1.
...
/// fault conditions.
///
/// See the DRV8461s datasheet for detailed descriptions of these conditions.
enum class DRV8461sFaultBit: uint8_t
{
...
///
/// See the DRV8461s datasheet for detailed descriptions of these conditions.
enum class DRV8461sDiag1Bit: uint8_t
{
...
///
/// See the DRV8461s datasheet for detailed descriptions of these conditions.
enum class DRV8461sDiag2Bit: uint8_t
{
...
enum class DRV8461sDiag3Bit: uint8_t
{
                //RSVD_7 = 7, // Reserved
...
enum class DRV8461sDecayMode: uint8_t
{
  Slow                   = 0b000,
...
enum class DRV8461sStepMode: uint8_t
{
...
/// This class provides high-level functions for controlling a DRV8461s stepper
/// motor driver.
class DRV8461s
{
public: 
  ...
  bool verifySettings()
  {
    return driver.readReg(DRV8461sRegAddr::CTRL1)  == ctrl1 &&
           driver.readReg(DRV8461sRegAddr::CTRL2)  == ctrl2 &&
           driver.readReg(DRV8461sRegAddr::CTRL3)  == ctrl3 &&
           driver.readReg(DRV8461sRegAddr::CTRL4)  == ctrl4 &&
           driver.readReg(DRV8461sRegAddr::CTRL5)  == ctrl5 &&
           driver.readReg(DRV8461sRegAddr::CTRL6)  == ctrl6 &&
           driver.readReg(DRV8461sRegAddr::CTRL7)  == ctrl7 &&
           driver.readReg(DRV8461sRegAddr::CTRL8)  == ctrl8 &&
           driver.readReg(DRV8461sRegAddr::CTRL9)  == ctrl9 &&
           driver.readReg(DRV8461sRegAddr::CTRL10) == ctrl10 &&
           driver.readReg(DRV8461sRegAddr::CTRL11) == ctrl11 &&
           driver.readReg(DRV8461sRegAddr::CTRL12) == ctrl12 &&
           driver.readReg(DRV8461sRegAddr::CTRL13) == ctrl13;
  }
  ...
  void applySettings()
  {
    writeCachedReg(DRV8461sRegAddr::CTRL2);
    writeCachedReg(DRV8461sRegAddr::CTRL3);
    writeCachedReg(DRV8461sRegAddr::CTRL4);
    writeCachedReg(DRV8461sRegAddr::CTRL5);
    writeCachedReg(DRV8461sRegAddr::CTRL6);
    writeCachedReg(DRV8461sRegAddr::CTRL7);
    writeCachedReg(DRV8461sRegAddr::CTRL8);
    writeCachedReg(DRV8461sRegAddr::CTRL9);
    writeCachedReg(DRV8461sRegAddr::CTRL10);
    writeCachedReg(DRV8461sRegAddr::CTRL11);
    writeCachedReg(DRV8461sRegAddr::CTRL12);
    writeCachedReg(DRV8461sRegAddr::CTRL13);

    // CTRL1 is written last because it contains the EN_OUT bit, and we want to
    // try to have all the other settings correct first.
    writeCachedReg(DRV8461sRegAddr::CTRL1);
  }
  ...
  /// Example usage:
  /// ~~~{.cpp}
  /// sd.setDecayMode(DRV8461sDecayMode::SmartTuneDynamicDecay);
  /// ~~~
  void setDecayMode(DRV8461sDecayMode mode)
  {
  ...
  void step()
  {
    driver.writeReg(DRV8461sRegAddr::CTRL2, ctrl2 | (1 << 6));
  }
  ...
  /// Example usage:
  /// ~~~{.cpp}
  /// sd.setStepMode(DRV8461sStepMode::MicroStep32);
  /// ~~~
  void setStepMode(DRV8461sStepMode mode)
  {
    if (mode > DRV8461sStepMode::MicroStep256)
    {
      // Invalid mode; pick 1/16 micro-step by default.
      mode = DRV8461sStepMode::MicroStep16;
    }
  ...
  void setStepMode(uint16_t mode)
  {
    ...
    switch (mode)
    {
      case 1:   sm = DRV8461sStepMode::MicroStep1;   break;
      case 2:   sm = DRV8461sStepMode::MicroStep2;   break;
      case 4:   sm = DRV8461sStepMode::MicroStep4;   break;
      case 8:   sm = DRV8461sStepMode::MicroStep8;   break;
      case 16:  sm = DRV8461sStepMode::MicroStep16;  break;
      case 32:  sm = DRV8461sStepMode::MicroStep32;  break;
      case 64:  sm = DRV8461sStepMode::MicroStep64;  break;
      case 128: sm = DRV8461sStepMode::MicroStep128; break;
      case 256: sm = DRV8461sStepMode::MicroStep256; break;

        // Invalid mode; pick 1/16 micro-step by default.
      default:  sm = DRV8461sStepMode::MicroStep16;
    }
  ...
  /// the #DRV8461sFaultBit enum to check individual bits.
  ///
  /// Example usage:
  /// ~~~{.cpp}
  /// if (sd.readFault() & (1 << (uint8_t)DRV8461sFaultBit::UVLO))
  /// {
  ...
  uint8_t readFault()
  {
    return driver.readReg(DRV8461sRegAddr::FAULT);
  }
  ...
  /// any of the bits are set, or you can use the logical AND operator (`&`) and
  /// the #DRV8461sDiag1Bit enum to check individual bits.
  uint8_t readDiag1()
  {
    return driver.readReg(DRV8461sRegAddr::DIAG1);
  }
  ...
  /// any of the bits are set, or you can use the logical AND operator (`&`) and
  /// the #DRV8461sDiag2Bit enum to check individual bits.
  uint8_t readDiag2()
  {
    return driver.readReg(DRV8461sRegAddr::DIAG2);
  }
  ...
  uint8_t readDiag3()
  {
    return driver.readReg(DRV8461sRegAddr::DIAG3);
  }
  ...
  uint8_t getCachedReg(DRV8461sRegAddr address)
  {
    ...
    /// for direct register accesses instead of calling DRV8461sSPI::writeReg()
    /// directly.
  void setReg(DRV8461sRegAddr address, uint8_t value)
  {
    uint8_t * cachedReg = cachedRegPtr(address);
    if (!cachedReg) { return; }
    *cachedReg = value;
...
protected: 
  ...
  uint8_t ctrl1, ctrl2, ctrl3, ctrl4, ctrl5, ctrl6, ctrl7, ctrl8, ctrl9, ctrl10, ctrl11, ctrl12, ctrl13;
    ...
    /// given register.
  uint8_t * cachedRegPtr(DRV8461sRegAddr address)
  {
    switch (address)
    {
      case DRV8461sRegAddr::CTRL1 : return &ctrl1;
      case DRV8461sRegAddr::CTRL2 : return &ctrl2;
      case DRV8461sRegAddr::CTRL3 : return &ctrl3;
      case DRV8461sRegAddr::CTRL4 : return &ctrl4;
      case DRV8461sRegAddr::CTRL5 : return &ctrl5;
      case DRV8461sRegAddr::CTRL6 : return &ctrl6;
      case DRV8461sRegAddr::CTRL7 : return &ctrl7;
      case DRV8461sRegAddr::CTRL8 : return &ctrl8;
      case DRV8461sRegAddr::CTRL9 : return &ctrl9;
      case DRV8461sRegAddr::CTRL10: return &ctrl10;
      case DRV8461sRegAddr::CTRL11: return &ctrl11;
      case DRV8461sRegAddr::CTRL12: return &ctrl12;
      case DRV8461sRegAddr::CTRL13: return &ctrl13;
           default: return nullptr;
    }
  }

    /// Writes the cached value of the given register to the device.
  void writeCachedReg(DRV8461sRegAddr address)
  {
    uint8_t * cachedReg = cachedRegPtr(address);
    if (!cachedReg) { return; }
